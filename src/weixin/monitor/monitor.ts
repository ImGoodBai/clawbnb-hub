...
export async function monitorWeixinProvider(opts: MonitorWeixinOpts): Promise<void> {
  ...
  while (!abortSignal?.aborted) {
    try {
      ...
      if (!firstPollLogged) {
        aLog.info(
          `monitor-ready: first getUpdates ok pollMs=${Date.now() - pollStartedAt} msgs=${resp.msgs?.length ?? 0} resumedSyncBuf=${previousGetUpdatesBuf ? "true" : "false"} timeoutMs=${nextTimeoutMs}`,
        );
